...
public final class MediaMetadataCompat implements Parcelable {
    ...
    public MediaDescriptionCompat getDescription() {
        ...
        for (Bitmap next2 : PREFERRED_BITMAP_ORDER) {
            Bitmap next22 = getBitmap(next22);
            if (next22 != null) {
                icon = next22;
                break;
            }
        }
        for (String next3 : PREFERRED_URI_ORDER) {
            String next32 = getString(next32);
            if (!TextUtils.isEmpty(next32)) {
                iconUri = Uri.parse(next32);
                break;
            }
        }
