...
public class VideoModule extends VideoBase implements OnErrorListener, OnInfoListener, ObjectViewListener, VideoRecordStateCallback {
    ...
    private int getHSRValue() {
        ...
        if (string == null || string.isEmpty() || string.equals("off") || isBeautyOn() || CameraSettings.isVideoBokehOn()) {
            return 0;
        }
        if (!isUsingUltraWideCamera() || DataRepository.dataItemFeature().fH()) {
            return Integer.parseInt(string);
        }
        Log.d(TAG, "60fps recording is force disabled for ultra wide");
        return 0;
    }
    ...
    private void startPreviewSession() {
        ...
        if (isDeviceAlive()) {
            ...
            this.mSurfaceCreatedTimestamp = this.mActivity.getCameraScreenNail().getSurfaceCreatedTimestamp();
            this.mCamera2Device.startPreviewSession(surface, false, false, 0, false, this);
        }
    }
    ...
    private String postProcessVideo(String str) {
        ...
        file3 = new File(Storage.generatePrimaryFilepath(file.getName()));
        obj = null;
        try {
