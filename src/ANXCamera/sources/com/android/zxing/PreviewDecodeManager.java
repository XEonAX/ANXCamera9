...
public class PreviewDecodeManager {
    ...
    private PreviewDecodeManager() {
        this.mPreviewCallback = new PreviewCallback() {
            public void onPreviewFrame(Image image, Camera2Proxy camera2Proxy, int i) {
                ...
                if (obj != null) {
                    PreviewImage previewImage = new PreviewImage(image, PreviewDecodeManager.this.mPreviewWidth, PreviewDecodeManager.this.mPreviewHeight, i);
                    Iterator it = arrayList.iterator();
                    while (it.hasNext()) {
                        ((Decoder) it.next()).onPreviewFrame(previewImage);
                    }
                }
            }
        };
    ...
    public void init(int i, int i2) {
        ...
        for (Entry value : this.mDecoders.entrySet()) {
            ((Decoder) value.getValue()).reConfig(i);
        }
    }
    ...
    public void stopDecode() {
        for (Entry value : this.mDecoders.entrySet()) {
            ((Decoder) value.getValue()).stopDecode();
        }
    }
