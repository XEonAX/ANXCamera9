...
public final class Detector {
    ...
    private static int getRotation(int[] sides, int length) throws NotFoundException {
        ...
        int i = ((cornerBits & 1) << 11) + (cornerBits >> 1);
        for (cornerBits = 0; cornerBits < 4; cornerBits++) {
            if (Integer.bitCount(EXPECTED_CORNER_BITS[cornerBits] ^ i) <= 2) {
                return cornerBits;
            }
        }
        throw NotFoundException.getNotFoundInstance();
    ...
    private ResultPoint[] getBullsEyeCorners(Point pCenter) throws NotFoundException {
        ...
        if (this.nbCenterLayers == 5 || this.nbCenterLayers == 7) {
            ...
            poutd = new ResultPoint(((float) pina.getX()) + 1056964608, ((float) pina.getY()) - 0.5f);
            pouta = new ResultPoint(((float) pinb.getX()) + 0.5f, ((float) pinb.getY()) + 0.5f);
            poutc = new ResultPoint(((float) pinc.getX()) - 0.5f, ((float) pinc.getY()) + 0.5f);
            ResultPoint poutb2 = new ResultPoint(((float) pind.getX()) - 0.5f, ((float) pind.getY()) - 0.5f);
            return expandSquare(new ResultPoint[]{poutd, pouta, poutc, poutb2}, (float) ((this.nbCenterLayers * 2) - 3), (float) (2 * this.nbCenterLayers));
        }
        throw NotFoundException.getNotFoundInstance();
    }
