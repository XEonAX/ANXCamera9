        ...
        if (codewords.length() == symbolInfo.getDataCapacity()) {
            ...
            if (blockCount == 1) {
                ...
                for (int block = 0; block < blockCount; block++) {
                    ...
                    for (i = block; i < symbolInfo.getDataCapacity(); i += blockCount) {
                        temp.append(codewords.charAt(i));
                    }
                    ...
                    while (e < errorSizes[block] * blockCount) {
                        int pos = i + 1;
                        sb.setCharAt(symbolInfo.getDataCapacity() + e, ecc.charAt(i));
                        e += blockCount;
                        i = pos;
                    }
                }
    ...
    private static String createECCBlock(CharSequence codewords, int start, int len, int numECWords) {
        ...
        if (table >= 0) {
            ...
            for (int i3 = start; i3 < start + len; i3++) {
                int m = codewords.charAt(i3) ^ ecc[numECWords - 1];
                i2 = numECWords - 1;
                while (i2 > 0) {
                    if (m == 0 || poly[i2] == 0) {
                        ecc[i2] = ecc[i2 - 1];
                    } else {
                        ecc[i2] = (char) (ecc[i2 - 1] ^ ALOG[(LOG[m] + LOG[poly[i2]]) % 255]);
                    }
                    i2--;
                }
                if (m == 0 || poly[0] == 0) {
            ...
            i2 = new char[numECWords];
            for (int i4 = 0; i4 < numECWords; i4++) {
                i2[i4] = ecc[(numECWords - i4) - 1];
            }
