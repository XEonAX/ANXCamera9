        ...
        while (i < max) {
            ...
            if (metadataString != null) {
                ...
                while (metadata3 < length) {
                    ...
                    if (metadata2 == null) {
                        metadata2 = new ArrayList(i2);
                    }
                    metadata2.add(metadatum);
                    String[] metadatumTokens = EQUALS.split(metadatum, 2);
                    matcher2 = matcher;
                    if (metadatumTokens.length > 1) {
                        key = metadatumTokens[0];
                        String value = metadatumTokens[1];
                        if ("ENCODING".equalsIgnoreCase(key) != null && "QUOTED-PRINTABLE".equalsIgnoreCase(value) != null) {
                            quotedPrintable2 = true;
                        } else if ("CHARSET".equalsIgnoreCase(key) != null) {
            ...
            if (i < 0) {
                ...
                if (i >= i3 && str.charAt(i - 1) == 13) {
                    i--;
                }
                key = str.substring(matchStart, i);
                if (trim) {
                    key = key.trim();
                }
                if (quotedPrintable) {
                    key = decodeQuotedPrintable(key, quotedPrintableCharset);
                    if (parseFieldDivider) {
                        key = UNESCAPED_SEMICOLONS.matcher(key).replaceAll("\n").trim();
                    }
                } else {
                    if (parseFieldDivider) {
                        key = UNESCAPED_SEMICOLONS.matcher(key).replaceAll("\n").trim();
                    }
                    key = VCARD_ESCAPES.matcher(NEWLINE_ESCAPE.matcher(CR_LF_SPACE_TAB.matcher(key).replaceAll("")).replaceAll("\n")).replaceAll("$1");
                }
                if (metadata == null) {
